#!/usr/bin/env bash
source conf.sh
FREE=`df -k --output=avail "$DIR" | tail -n1`   # df -k not df -h
if [[ ${FREE} -lt 26214400 ]]; then               # 25G = 25*1024*1024k
     echo "for building need ~25G free space"; exit 1;
fi;
python2.7 ${GENVAR} ${PACKER_VAR_FILE} || { echo 'generating variables failed' ; exit 1; }
CREATED_AT=$(python2.7 $GETVAR ${PACKER_VAR_FILE} created_at)
export BLACKARCH_PROFILE=core
packer-io build -var-file=${PACKER_VAR_FILE} -only=virtualbox-iso ${PACKER_TEMPLATE} && \
    vagrant up && \
...
vagrant destroy -f
sleep 5
packer-io build -var-file=${PACKER_VAR_FILE} -only=qemu ${PACKER_TEMPLATE}