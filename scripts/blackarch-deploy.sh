...
install_base()
{
    ...
    /usr/bin/pacstrap ${CHROOT} bash bzip2 coreutils cryptsetup device-mapper dhcpcd diffutils e2fsprogs file \
                                filesystem findutils gawk gettext glibc grep gzip inetutils iproute2 \
                                iputils jfsutils less licenses logrotate lvm2 man-db man-pages mdadm nano netctl \
                                pacman pciutils perl procps-ng psmisc reiserfsprogs s-nail sed shadow sysfsutils \
                                systemd-sysvcompat tar texinfo usbutils util-linux vi which xfsprogs sudo \
                                archlinux-keyring > /dev/null
    ...
    /usr/bin/arch-chroot ${CHROOT} pacman -S --noconfirm  --quiet --noprogressbar --needed \
                                             autoconf automake binutils bison fakeroot flex libtool m4 make patch \
                                             pkgconf ruby ruby-pkg-config cmake gcc-multilib > /dev/null
...
add_vagrant_user_chroot(){
    ...
    cp -r ${BI_PATH}/data/user/. ${CHROOT}/home/${USER_NAME}/.  # customise vagrant environment
    /usr/bin/arch-chroot ${CHROOT} /bin/chown -R ${USER_NAME}:${USER_NAME} /home/vagrant/
}
...
install_virtualbox_guest(){
    ...
    /usr/bin/pacman -S --noconfirm ${KERNEL}-headers virtualbox-guest-utils-nox virtualbox-guest-dkms nfs-utils
    ...
    if getent passwd ${USER_NAME} > /dev/null 2>&1; then
        ...
        /usr/bin/getent group ${USER_NAME} || /usr/bin/groupadd ${USER_NAME}
